...
mod verifier_fri_e2f2;
pub use verifier_fri_e2f2::*;
...
fn fri_fold4_ext2() {
    ...
    let (advice_provider, tape, alphas, commitments, remainder, num_queries) =
        ...
        //Result<((Vec<AdviceSet>, Vec<([u8; 32], Vec<Felt>)>), Vec<u64>, Vec<u64>, Vec<u64>, (u64, u64), usize), VerifierError>
        //Result<((Vec<AdviceSet>, Vec<([u8; 32], Vec<Felt>)>), Vec<u64>, Vec<u64>, Vec<u64>, (u64, u64), usize), VerifierError>
...
fn prepare_advice(
    ...
    for i in (0..(depth - blowup_exp)).rev() {
        ...
        println!("alpha {:?}",& alphas[(4 * i)..(4 * i + 2)]);
    }
...
fn prepare_advice_fold4(
    ...
    for i in (0..(depth - blowup_exp)).rev() {
        ...
        tape.extend_from_slice(&vec![current_domain_size, current_depth]);
